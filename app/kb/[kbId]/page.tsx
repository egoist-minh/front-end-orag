...
export default function KBDocumentsStatus() {
  ...
  const { kbDocs, deleteKbDoc, kbs } = useAppStore();
    ...
    <div className="flex flex-col h-full bg-background overflow-y-auto p-10 relative">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-primary tracking-tight mb-2">
          {currentKb ? `Tài liệu: ${currentKb.name}` : 'Tài liệu & Trạng thái'}
        </h1>
              ...
              uploadedDocs.map(doc => (
                <div key={doc.id} className="flex items-center justify-between py-4 border-b border-surface-variant group">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => deleteKbDoc(kbId, doc.id)} className="text-surface-variant hover:text-error transition-colors p-2">
